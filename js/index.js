...
window.addEventListener('scroll', checkWindowYOffset)

function checkWindowYOffset() {
    /**
     * Elemento <<a>nav>
     * @description Navbar
     * @type {HTMLElement}
...
function resizeContent() {
    ...
    if (window.matchMedia("(min-width: 1920px)").matches) { //Se a largura da página for maior ou igual que 1920px
        ...
        window.removeEventListener('scroll', checkWindowYOffset);
    ...
    } // Se a largura da página for maior ou igual que 992px e menor que 1919px
    else if (window.matchMedia("(min-width: 992px) and (max-width:1199px)").matches) { // If media query matches
